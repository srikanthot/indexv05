...
def _derive_taxonomy_from_path(source_path: str, source_file: str) -> dict[str, Any]:
    """Best-effort taxonomy from the path/filename when blob metadata is
    absent. Deliberately conservative — only sets a value on a clear keyword
    match, else leaves None so nothing wrong gets stamped."""
    blob = f"{source_path or ''} {source_file or ''}".lower()
    op = None
    if "gas" in blob:
        op = "Gas"
    elif "electric" in blob or "elec" in blob or "t&d" in blob:
        op = "Electric"
    elif "substation" in blob:
        op = "Substation"
    doctype = None
    for kw, dt in (
        ("bulletin", "Bulletin"), ("standard", "Standard"), ("procedure", "Procedure"),
        ("manual", "Manual"), ("spec", "Specification"), ("guide", "Guide"),
    ):
        if kw in blob:
            doctype = dt
            break
    return {"operationalarea": op, "functionalarea": None, "doctype": doctype}
...
def process_document(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not source_path:
        return {
            ...
            "operationalarea": None,
            "functionalarea": None,
            "doctype": None,
            "title": None,
        }
 
    # Fetch user-defined blob metadata (taxonomy fields).
    # This is the reliable path — enrichment tree doesn't surface
    # user-defined metadata for index projections.
    blob_meta = fetch_blob_metadata(source_path)
    # Blob user-metadata is authoritative when present; otherwise derive a
    # best-effort fallback from the path/filename so taxonomy fields aren't
    # simply null (they are anti-contamination scoping keys). Operators can
    # still override by setting real blob metadata.
    derived = _derive_taxonomy_from_path(source_path, source_file)
    taxonomy = {
        "operationalarea": blob_meta.get("operationalarea") or derived.get("operationalarea"),
        "functionalarea": blob_meta.get("functionalarea") or derived.get("functionalarea"),
        "doctype": blob_meta.get("doctype") or derived.get("doctype"),
        # Operator-set blob metadata "title" (no path fallback — leave null if
        # unset). Surfaced as the `title` index field like the other taxonomy keys.
        "title": blob_meta.get("title"),
    }
